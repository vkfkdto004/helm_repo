...
v2.4 (2026.01.20) - 개발 중에 이제 CI/CD 파이프라인을 구축하고 기능 추가하는 실제 현업에서 하는것과 같이 진행하기 전에 ELK 스택을 ECK(elastic container in kubernetes)를 이용해서 한번 구축했다. 내부적으로 TLS 구성이 다되어 있는 환경으로 보안에 신경을 써서 구성을 했으며 현재 k8s 환경이 cilium을 CNI로 쓰고 있는지라 gateway랑 route 부분에서 애를 좀 먹었다. <br>
실제 cilium에서 제공하는 envoy를 사용하면 아래와 같은 로직을 가진다. <br>
...
v 2.5 (2026.01.27) - elastic ML 교육 수강 후에 elastic에 대한 전반적인 아키텍쳐를 좀 더 알게 되었다. 기본적으로 elastic을 다루거나 이론적인 부분은 유튜브 강의를 통해 채우고 있고 monitoring 및 observability는 아래와 같이 beat를 나눠서 구성을 하였다.
```
...
---
v3.0 (2026.02.03) - 현재 MySQL operator on kubernetes 메뉴얼을 보고 구성중에 있음. 이전에는 구성하는 방법을 개인적으로 보관했는데, 잘 정리해서 블로그에 올리고자 블로그 작성도 시작하였음.
디테일하게 에러를 어떻게 해결했는지, 어떻게 관찰했는지 등 자세하게 적을 것이니 참고..
https://container-infra.tistory.com/
